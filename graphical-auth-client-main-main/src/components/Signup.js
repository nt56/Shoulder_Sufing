...
export default function Signup(props) {
  ...
  const [inputIteration, setInputIteration] = useState(1);
  ...
  function handleInput(e) {
    ...
    newArray[inputIteration] = e.target.value;
    setKeyword(newArray);
  }
  ...
  function getIcons() {
    return (
      storeImage[iteration] &&
      storeImage[iteration].map((prev) => (
        <PasswordIcon
          iteration={iteration}
          id={prev.id}
          key={nanoid()}
  ...
  function createAccount() {
    ...
    if (iteration < 3) {
      ...
      setInputIteration(inputIteration + 1);
      return;
    }
  ...
  function searchKeyword() {
    ...
    axios
      .get(`${api.url}/api/image/search?keyword=${keyword[inputIteration]}`)
      .then((data) => {
        props.setLoading(false);
        setImageData(data.data[0]);
        if (storeImage[iteration] && storeImage[iteration].length != 0) {
          const newArray = [...storeImage];
          newArray[iteration] = data.data[0];
          setStoreImage(newArray);
        } else {
          setStoreImage((prevArray) => [...prevArray, data.data[0]]);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <div className=" sm:h-[38rem] mt-12">
      {!next && (
        <div className="flex justify-center h-full">
          ...
          <div className="font-['Work_Sans'] mt-4">
            <p className="px-4 sm:px-0 text-white text-3xl sm:text-5xl sm:font-bold">Create Account</p>
            <br />
            <p className="text-white text-lg sm:text-2xl px-4 sm:px-0">Welcome! Enter Your Details And Experience</p>
            <p className="text-white text-lg sm:text-2xl px-4 sm:px-0">Graphical Password System.</p>
            <br />
            <div className="flex flex-col w-[80%] sm:w-2/3 px-4 sm:px-0">
              <input
                ...
                name="username"
                className="rounded-full h-8 sm:h-12 px-6 font-3xl"
                type="text"
              ...
              <input
                ...
                onChange={handleChange}
                name="email"
                className="rounded-full h-8 sm:h-12 px-6 font-3xl mt-4"
                type="email"
                placeholder="Email"
              />
              <input
            ...
            <button
              ...
              className="ml-4 sm:ml-0 transition duration-500 ease-in-out h-8 sm:h-12 bg-[#A259FF] rounded-full px-6 sm:w-2/3 mt-6 text-white border-2 hover:bg-transparent border-[#A259FF] font-bold">
              Next
            </button>
          </div>
        </div>
      ...
      {next && (
        <div className="sm:flex h-auto">
          {storeImage.length >= 1 && storeImage[iteration] ? (
            <div className="hidden sm:grid grid-cols-4 bg-[#3B3B3B] h-full rounded-lg w-[75%] justify-items-center py-4 px-2 gap-2 ml-12">{getIcons()}</div>
          ) : null}
          {storeImage.length == 0 || storeImage[iteration] == undefined ? (
          ...
          <div className="hidden sm:block font-['Work_Sans'] mt-4 ml-12">
            ...
            <p className="text-white text-2xl">
              Select <span className="text-green-400">{getNameByNumber(iteration + 1)}</span> Image.
            </p>
            <br />
            {/* {iteration === 0 && ( */}
            <div className="align-middle items-center">
              ...
              <div className=" rounded-lg flex mt-2">
                <input
                  ...
                  value={keyword[inputIteration] ? keyword[inputIteration] : ""}
                  placeholder="Try 'Cats'"
                  className="rounded-l-md px-4 bg-gray-100 text-2xl py-1"
                />
                <button onClick={searchKeyword} className="bg-gray-100 transition duration-500 ease-in-out rounded-r-lg px-4 h-12 hover:bg-gray-300">
          ...
          <div className="sm:hidden font-['Work_Sans'] mt-4 ml-4">
            ...
            <div className="align-middle items-center">
              ...
              <div className=" rounded-md flex mt-2">
                ...
                />
                <button onClick={searchKeyword} className="bg-gray-100 transition duration-500 ease-in-out rounded-r-lg px-4 h-8 hover:bg-gray-300">
                  <FontAwesomeIcon className="text-black" icon={faSearch} />
                </button>
              </div>
            </div>
            ...
            {storeImage.length >= 1 && storeImage[iteration] ? (
              <div className="mt-4 grid grid-cols-2 bg-[#3B3B3B] h-full rounded-md w-full justify-items-center py-4 gap-1 gap-x-0 -ml-2">{getIcons()}</div>
            ) : null}
            {storeImage.length == 0 || storeImage[iteration] == undefined ? (
