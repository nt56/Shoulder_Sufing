...
export default function Login(props) {
  ...
  async function handleNextClick(event) {
    if (validateData() && (await validateUsernameAndEmail())) {
      axios
        .get(`${api.url}/api/image?username=${loginInfo.username}`)
        .then((res) => {
  ...
  function login() {
    ...
    axios
      .post(`${api.url}/api/user/login`, loginInfo)
      .then((res) => {
  ...
  return (
    <div className=" sm:h-[38rem] sm:mt-12">
      ...
      {!next && (
        <div className="flex justify-center h-full">
          ...
          <div className="font-['Work_Sans'] mt-16">
            <p className="text-white text-3xl sm:text-5xl sm:font-bold px-4 sm:px-0">Login</p>
            <br />
      ...
      {next && (
        <div className="sm:flex justify-center h-auto ">
          ...
          <div className="sm:hidden font-['Work_Sans'] mt-4 ml-4">
            <p className="text-white text-2xl font-bold">Set Graphical Password</p>
            <br />
            ...
            <div className="-ml-4 grid grid-cols-2 bg-[#3B3B3B] gap-x-0 h-full rounded-md justify-items-center py-4 gap-1">{getIcons()}</div>
